...
int jprobe_tcp_close_entry(struct sock *sk) {
    ...
    do {
        ...
        printk(
            ...
            "\"segs_in\":%u,\"segs_out\":%u,\"delivered\":%u,\"lost\":%u,\"total_retrans\":%u,"
            ...
            tp->segs_in, tp->segs_out, tp->delivered, tp->lost, tp->total_retrans,
...
static struct jprobe jprobe_tcp_close = {
    .kp = {
        .symbol_name = "tcp_close",
    },
    .entry = jprobe_tcp_close_entry,
};
 
static __init int jprobe_tcp_close_init(void)
{
    register_jprobe(&jprobe_tcp_close);
    printk("jprobe_tcp_close installed\n");
    return 0;
}
 
static __exit void jprobe_tcp_close_exit(void)
{
    unregister_jprobe(&jprobe_tcp_close);
    printk("jprobe_tcp_close removed\n");
}
 
module_init(jprobe_tcp_close_init);
module_exit(jprobe_tcp_close_exit);
