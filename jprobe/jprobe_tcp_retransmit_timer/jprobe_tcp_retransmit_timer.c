...
int jprobe_tcp_retransmit_timer_entry(struct sock *sk) {
...
static struct jprobe jprobe_tcp_retransmit_timer = {
    .kp = {
        .symbol_name = "tcp_retransmit_timer",
    },
    .entry = jprobe_tcp_retransmit_timer_entry,
};
 
static __init int jprobe_tcp_retransmit_timer_init(void)
{
    register_jprobe(&jprobe_tcp_retransmit_timer);
    printk("jprobe_tcp_retransmit_timer installed\n");
    return 0;
}
 
static __exit void jprobe_tcp_retransmit_timer_exit(void)
{
    unregister_jprobe(&jprobe_tcp_retransmit_timer);
    printk("jprobe_tcp_retransmit_timer removed\n");
}
 
module_init(jprobe_tcp_retransmit_timer_init);
module_exit(jprobe_tcp_retransmit_timer_exit);
